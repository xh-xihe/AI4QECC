...
lambda2_list = np.linspace(0.0, 5, 500)
# 确保关键目标点在网格中
...
def main():
    ...
    for lambda2 in lambda2_list:
        ...
        with torch.no_grad():
            ...
            for h in callback.history_state:
                ...
                lam_vec = diagA.mean(dim=1)                       # [M]  每个算符的 λ_k
    ...
    # ====== (B) 图上标注关键点（例如 0.6 与 1.0） ======
    annotate_xs = [0.6, 1.0]
    for x0 in annotate_xs:
        # 找到目标点索引
        j = np.where(np.isclose(lambda2_list, x0))[0]
        if len(j) == 0: 
            continue
        j = j[0]
        y = fval_optim[j, 3]
        if not np.isfinite(y):
            continue
        ax.scatter([x0],[y], zorder=3)
        ax.annotate(f"{y:.2e}", xy=(x0, y), xytext=(6, 6),
    ...
    # ====== (C) 保存 CSV：target, total, offdiag, diag, lambda_loss, achieved_lambda2 ======
    out_csv = f"{out_base}.csv"
    # 如果 fval_optim 某些行是 nan，适当处理
